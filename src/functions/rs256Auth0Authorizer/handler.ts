...
const certificateUrl = `https://${auth0Domain}/.well-known/jwks.json`;

// Automating certificate retrieval @getCertificate
...
async function verifyToken(authHeader: string): Promise<JwtToken> {
  ...
  const token = split[1]
  const cert = `-----BEGIN CERTIFICATE-----\n${(await getCertificate(certificateUrl))}\n-----END CERTIFICATE-----`;

  return verify(token, cert, { algorithms: ['RS256'] }) as JwtToken
}

const getCertificate = async (certificateUrl) => {
  try {
